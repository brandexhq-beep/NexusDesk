...
export function Promotions() {
  ...
  const loadData = async () => {
    ...
    // 1. Clean up any existing duplicate templates in DB by name
    const seenNames = new Set<string>();
    const duplicatesToDelete: string[] = [];
    for (const tmpl of t) {
      if (seenNames.has(tmpl.name)) {
        duplicatesToDelete.push(tmpl.id);
      } else {
    ...
    if (duplicatesToDelete.length > 0) {
      for (const dupId of duplicatesToDelete) {
        await db.templates.delete(dupId);
      }
      t = await db.templates.getAll();
    }

    // 2. Define default templates & seed ONLY missing ones by name
    ...
    const currentNames = new Set(t.map(item => item.name));
    let seeded = false;
    for (const defTmpl of defaultTemplates) {
      if (!currentNames.has(defTmpl.name)) {
        await db.templates.add(defTmpl);
        seeded = true;
      }
    }
